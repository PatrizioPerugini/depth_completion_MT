...
void optimize_IG(cv::Mat & entryMatrix_left, cv::Mat & entryMatrix_right, cv::Mat& disparity_map_IG){
        ...
        for (int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                ...
                if(ok && disparity_map_IG.at<float>(i,j)!=0){
                    ...
                    //std::cout<<"Value: "<<value<<", Derivative: (" <<derivative.x()<<", "<<derivative.y() << ")" << std::endl;
                    if (error > 255){
                        error = 255;
                    }
                    if (error < -255){
...
//convert the depth into a disparity using baseline and focal length
...
void reproject_pc(cv::Mat& depth_image, Eigen::Matrix4f& R){
    ...
    for (int y = 0; y < depth_image.rows; ++y) {
        for (int x = 0; x < depth_image.cols; ++x) {
            ...
            if (depth > 0) {
                ...
                float y_ = (y - cy) * z / fy;
                //
                //Eigen::Vector3f p(x_ , y_ , z);
                //Eigen::Vector4f transformedpc = R * p.homogeneous();
                //x_=transformedpc(0);
                //y_=transformedpc(1);
                //z=transformedpc(2);
...
void unrectify_sol(cv::Mat& depth_pre_optim, cv::Mat& depth_pre_optim_unrect, 
                    Eigen::Matrix4f& R_rect, cv::Mat& image){
    double fx = 9.597910e+02;  
    double fy =  9.569251e+02; 
    double cx = 6.960217e+02;; 
    double cy = 2.241806e+02; 
    Eigen::Matrix3f camera_mat;
    camera_mat << 9.597910e+02, 0.000000e+00, 6.960217e+02,
                  0.000000e+00, 9.569251e+02, 2.241806e+02, 
                  0.000000e+00, 0.000000e+00, 1.000000e+00;

    Eigen::Matrix<float, 3, 4> p_mat;
    p_mat << 7.215377e+02, 0.000000e+00, 6.095593e+02, 4.485728e+01,
             0.000000e+00, 7.215377e+02, 1.728540e+02, 2.163791e-01, 
             0.000000e+00, 0.000000e+00, 1.000000e+00, 2.745884e-03;

    // Generate the point cloud
    for (int y = 0; y < depth_pre_optim.rows; ++y) {
        for (int x = 0; x < depth_pre_optim.cols; ++x) {
            float depth = static_cast<float>(depth_pre_optim.at<float>(y, x));// / 1000.0;
            //unproject
            float z = depth;
            float x_ = (x - cx) * z / fx;
            float y_ = (y - cy) * z / fy;
            ...
            Eigen::Vector4f transformedpc = R_rect.inverse() * p.homogeneous();
            Eigen::Vector3f transformed_point = transformedpc.head<3>(); 
            //Eigen::Vector3f transformed_point = p_mat*transformedpc;
            float transformed_depth = transformedpc(2);
            //float transformed_depth = transformed_point(2);

            if (transformed_depth > 0) {
                Eigen::Vector3f camera_point = camera_mat * transformed_point;
                Eigen::Vector2f image_point = camera_point.head<2>()*1.f/transformed_depth;
                //Eigen::Vector2f image_point = transformed_point.head<2>()*1.f/transformed_depth;                
                transformedpc(0)=image_point(0);
                transformedpc(1)=image_point(1);
                //project on the rectified frame if possible
                //std::cout <<"point      : " << p.transpose() <<std::endl;
                std::cout <<"transformed: " << transformedpc.transpose() <<std::endl;
                if (transformedpc(0) >= 0 && transformedpc(0) < depth_pre_optim_unrect.cols &&
                    transformedpc(1) >= 0 && transformedpc(1) < depth_pre_optim_unrect.rows){
                       
                int u = static_cast<int>(transformedpc(0));
                int v = static_cast<int>(transformedpc(1));
                 
                cv::circle(image, cv::Point(u, v), 2, cv::Scalar(255, 255, 255), -1);
                depth_pre_optim_unrect.at<float>(v, u) = transformedpc(2);
                //std::cout << "transformed: " << transformedpc(2) <<std::endl;
                //std::cout << "original: " << depth <<std::endl;
                }
            }
        }
    }
    //cv::circle(image, cv::Point(200, 200), 2, cv::Scalar(255, 255, 255), -1);
    //cv::Mat IG_rectified_;
    //cv::normalize(IG_rectified, IG_rectified_, 0, 100, cv::NORM_MINMAX);
    cv::imshow("rcd", image);
...
void evaluate_performances(cv::Mat& GT_img, cv::Mat& r_img, float& mae, float& rmse){
    ...
    for (int i = 0; i < rows; i++){
        for(int j = 0; j < cols; j++){
            ...
            //std::cout << "gt: " << gt_value <<std::endl;
            //std::cout << "r: " << r_value <<std::endl;

            
            if ( GT_img.at<float>(i,j) > tolerance && r_value > tolerance){
                
                float d_err = fabs(gt_value - r_value);
                //std::cout << "WELA" << d_err <<std::endl;
...
int main() {
    ...
    Eigen::Matrix4f T_V_2_C0, T_C0_2_C2, T_C0_2_C3;
    
    
    
    Eigen::Matrix4f R_rect_02, R_rect_03, R_rect_00;
    ...
    */
    ...
    Eigen::Matrix4f T3 = R_rect_03*T_C0_2_C3.inverse()*T_V_2_C0;
...
//
    ...
    //}
       ...
       //-------------------------------- EVALUATION -----------------------
    std::string image_raw_gt = "../depth_selection/val_selection_cropped/groundtruth_depth/2011_09_26_drive_0002_sync_groundtruth_depth_0000000005_image_02.png";
    std::string imgage_not_rectified_path = "../depth_selection/val_selection_cropped/image/2011_09_26_drive_0002_sync_image_0000000005_image_02.png";
    cv::Mat img_left_not_rect = cv::imread(imgage_not_rectified_path);
    
    cv::Mat image_r_gt = cv::imread(image_raw_gt, cv::IMREAD_ANYDEPTH);
    if (image_r_gt.empty()) {
    ...
    cv::Mat GT;
    image_r_gt.convertTo(GT, CV_32F, 1.0 / 256.0);
   
    
    cv::Mat depth_pre_optim=cv::Mat::zeros(img_left.rows, img_left.cols, CV_32F);
    retrieve_optimized_depth(disparity_IG, depth_pre_optim);
    cv::Mat depth_pre_optim_unrect=cv::Mat::zeros(img_left_not_rect.rows, img_left_not_rect.cols, CV_32F);
    unrectify_sol(depth_pre_optim, depth_pre_optim_unrect,R_rect_02, img_left_not_rect);
    //cv::warpPerspective(image_r_gt, post_IG, P_rect_02, (img_left.cols, img_left.rows), cv::INTER_LINEAR);
    float mae_pre, rmse_pre, mae_post;
    
    evaluate_performances(image_r_gt, depth_pre_optim_unrect, mae_pre, rmse_pre);

    std::cout << "MAE pre is: " << mae_pre << std::endl;


    //--------------------------------
    
    
    optimize_IG(entryMatrix_left,entryMatrix_right,disparity_IG);
    ...
    evaluate_performances(image_r_gt, optimized_depth, mae_post, rmse_pre);

    std::cout << "MAE post is: " << mae_post << std::endl;

    cv::Mat optimized_depth_color ;
    cv::Mat optimized_depth_color_show;
    cv::GaussianBlur(optimized_depth, optimized_depth, cv::Size(5, 5), 0);
